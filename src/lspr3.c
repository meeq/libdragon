...
static void lspr3_decode_intra_slice(
    ...
    // mb storage is transient — allocated here, freed at the end of this
    // function. It is sourced through the tagged seam (SYS_ALLOC_LSPR3_MB); the
    // weak default is memalign on the main heap, and an application can override
    // __sys_alloc to route it elsewhere. The seam does not zero the buffer, so
    // memset it here (the decoder relies on the mb array being zero-initialised).
...
static void lspr3_decode_intra_slice_banded(
    ...
    memset(mb, 0, mb_bytes);   // the decoder relies on zero-initialised mb storage
    h264bsdInitMbNeighbours(mb, (u32)mb_w, pic_size_in_mbs);
                            ...
                            const lspr3_load_parms_t *parms)
{
    ...
    // Allocate the buffer for the decoded sprite through the tagged seam
    // (SYS_ALLOC_LSPR3_SPRITE), aligned to H264I_BUF_ALIGN so the post-header
    // pixel area lands on a 64-byte boundary (required by rdpq_set_color_image
    // when the pixel area is used as a YUV blit target). The weak default is
    // memalign, so a sprite from the standard path may be released with
    // sprite_free(); an application that overrides __sys_alloc owns the matching
    // __sys_free(SYS_ALLOC_LSPR3_SPRITE) and must not use sprite_free() on it.
    ...
    if (band_rows > 0) {
        ...
        lspr3_decode_intra_slice_banded(
            ...
            NULL, sprite, band_rows, x_divisor, y_divisor);
    } else {
        ...
        lspr3_decode_intra_slice(
            hdr->payload, payload_size, hdr->width, hdr->height,
            hdr->pic_init_qp, hdr->chroma_qp_index_offset, &pic, &pic_size);
    ...
    {
        sprite_t *spr2 = (sprite_t*)__sys_alloc(SYS_ALLOC_LSPR3_SPRITE, decoded_sz, H264I_BUF_ALIGN);
        if (spr2) {
            ...
            __sys_free(SYS_ALLOC_LSPR3_SPRITE, spr2);
        } else {
