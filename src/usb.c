...
static void usb_64drive_write(int datatype, const void* data, int size)
{
    ...
    while (left > 0)
    {
        ...
        // Set up DMA transfer between RDRAM and the PI
            ...
            cart_dma_write(usb_buffer, D64_BASE_ADDRESS + DEBUG_ADDRESS + read, block);
        #else
...
static void usb_64drive_read()
{
    // Set up DMA transfer between RDRAM and the PI
        ...
        cart_dma_read(usb_buffer, D64_BASE_ADDRESS + DEBUG_ADDRESS + usb_readblock, BUFFER_SIZE);
    #else
...
static void usb_everdrive_read()
{
    // Set up DMA transfer between RDRAM and the PI
        ...
        while (dma_busy());
        *(vu32*)0xA4600010 = 3;
        cart_dma_read(usb_buffer, ED_BASE + DEBUG_ADDRESS + usb_readblock, BUFFER_SIZE);
        data_cache_hit_writeback_invalidate(usb_buffer, BUFFER_SIZE);
    #else
        osWritebackDCacheAll();
        #if USE_OSRAW
...
static void usb_sc64_write(int datatype, const void* data, int size)
{
    ...
    while (left > 0)
    {
        ...
        // Write data to buffer in SDRAM
            ...
            cart_dma_write(usb_buffer, sdram_address, dma_length);
        #else
...
static u32 usb_sc64_poll(void)
{
    ...
    if (fifo_items >= 2)
    {
        ...
        while (left > 0)
        {
            ...
            #ifdef LIBDRAGON
                cart_dma_read(usb_buffer, SC64_MEM_USB_FIFO_BASE, dma_length);
                cart_dma_write(usb_buffer, sdram_address, dma_length);
            #else
                #if USE_OSRAW
...
static void usb_sc64_read(void)
{
    ...
    // Set up DMA transfer between RDRAM and the PI
    #ifdef LIBDRAGON
        cart_dma_read(usb_buffer, sdram_address, BUFFER_SIZE);
