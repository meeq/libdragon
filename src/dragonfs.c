...
static inline void grab_sector(void *cart_loc, void *ram_loc)
{
    ...
    cart_dma_read((void *)(((uint32_t)ram_loc) & 0x1FFFFFFF), (uint32_t)cart_loc, SECTOR_SIZE);
}
...
int dfs_read(void * const buf, int size, int count, uint32_t handle)
{
    ...
    if (rom_aligned && ram_aligned && len_aligned)
    {
        ...
        cart_dma_read((void *)(((uint32_t)buf) & 0x1FFFFFFF),
    ...
    while(to_read)
    {
        ...
        if (file->loc < file->cached_loc || file->loc >= file->cached_loc+CACHED_SIZE)
        {
            ...
            cart_dma_read((void *)(((uint32_t)file->cached_data) & 0x1FFFFFFF),
 ...
 * 
 * Direct access to ROM data must go through io_read or cart_dma_read. Do not
